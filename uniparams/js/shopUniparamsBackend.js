var getthelist = function(li_id = undefined) {
    $.ajax({
        ...
        $('.uniparams-thelist').nestedSortable({
            axis: 'y',
            helper: "clone",
            handle: 'div',
            items: 'li',
            ...
            isAllowed: function (placeholder, placeholderParent, currentItem) {
                if ($(currentItem).attr('data-is-group') == 1 && $(placeholderParent).attr('data-is-group') == 1)
                    return false;
                // console.log($(currentItem).attr('data-is-group'));
                // console.log($(placeholderParent).attr('data-is-group'));
                return true;
            },
...
var getitems = function(li_id = undefined) {
    ...
    if (li_id == undefined) {
        $('.uniparams-thelist li[data-is-group="0"]:first').addClass('selected');
        li_id = $('.uniparams-thelist li[data-is-group="0"]:first').attr('data-id');
    } else {
...
$(document).ready(function() {
    // lists (sidebar) (add-list button)
    $(document).off('click', '.uniparams-add_list').on('click', '.uniparams-add_list', function(e) {
        e.preventDefault();
        let dialog = `
    ...
    // lists (sidebar) (add-group button)
    $(document).off('click', '.uniparams-add_group_list').on('click', '.uniparams-add_group_list', function(e) {
        e.preventDefault();
        let dialog = `
            <div class="uniparams-newgrouplist dialog-content" style="display: none;">
                <div class="dialog-content-indent">
                    <h1>Новый group</h1>
                    <form>
                        <div class="block form">
                            <div class="field-group">
                                ...
                                <div class="field">
                                    <div class="name">Код вывода <br><span class="hint">id</span></div>
                                    <div class="value">{shopUniparamsPlugin::getGroup("<input type="text" name="list_key"/>")}
                                        <em class="errormsg uniparams-err1"></em>
                                    </div>
                                </div>
                                <div class="field">
                ...
                <div class="dialog-buttons block">
                    <input type="submit" value="Сохранить" class="button green uniparams-create_group_list" />
                    <button class="button gray cancel">Закрыть</button>
                </div>
            </div>
            `;
        ...
        $('.uniparams-newgrouplist input[name="list_key"]').translit('watch', '.uniparams-newgrouplist input[name="title"]', true, true);
    });
    // Add new field list popup
    $(document).off('click', ".uniparams-add-list-field").on('click', ".uniparams-add-list-field", function(e) {
        e.preventDefault();
    ...
    $(document).off('click', '.uniparams-delete-list-field').on('click', '.uniparams-delete-list-field', function(e){
        e.preventDefault();
    ...
    $(document).off('click', ".uniparams-add-list-param").on('click', ".uniparams-add-list-param", function(e) {
        e.preventDefault();
    ...
    $(document).off('click', '.uniparams-delete-list-param').on('click', '.uniparams-delete-list-param', function(e){
        e.preventDefault();
        $(this).parents(".field").remove();
    });

    // create list
    $(document).off('click', '.uniparams-create_list').on('click', '.uniparams-create_list', function(e) {
        e.preventDefault();
    ...
    // edit/save list
    $(document).off('click', '.uniparams-save_list').on('click', '.uniparams-save_list', function(e) {
        e.preventDefault();
    ...
    // delete list
    $(document).off('click', '.uniparams-remove_list').on('click', '.uniparams-remove_list', function(e) {
        e.preventDefault();
    ...
    // popup edit list
    $(document).off('click', '.uniparams-list-settings').on('click', '.uniparams-list-settings', function(e) {
        e.preventDefault();
    ...
    // select/open list
    $(document).off('click', '.uniparams-thelist li[data-is-group="0"]').on('click', '.uniparams-thelist li[data-is-group="0"]', function(e) {
        e.preventDefault();
        getitems($(this).attr('data-id'));
    });
    // change input type list params
    $(document).off('change', ':input[name="lptype[]"]').on('change', ':input[name="lptype[]"]', function(e) {
        e.preventDefault();
    ...
    $(document).off('change', '.uniparams-list-params input[type="file"]').on('change', '.uniparams-list-params input[type="file"]', function () {
        let that = $(this);
        var oFReader = new FileReader();
    ...
    // create list group
    $(document).off('click', '.uniparams-create_group_list').on('click', '.uniparams-create_group_list', function(e) {
        e.preventDefault();

        var form_data = new FormData();
        ev_fine = true;
        $('.uniparams-newgrouplist').find('.errormsg').html('');
        $(".uniparams-newgrouplist :input").each(function(index) {
            that = $(this);
            if (that.attr('type') == 'file' && that.prop('files')[0]) {
                var name = that.prop('files')[0].name;
                var ext = name.split('.').pop().toLowerCase();
        ...
        if (ev_fine) {
            $.ajax({
                ...
                cache: false,
                processData: false,
                url: '?plugin=uniparams&action=addgrouplist'
            }).done(function (data) {
                if (data.status == "ok") {
                    $('.uniparams-newgrouplist').trigger('close');
                    $('.uniparams-newgrouplist').remove();
                    getthelist(data.data.list_id);
                } else {
                    if (data.errors.id)
                        $('.uniparams-err1').html('').html(data.errors.id);
    ...
    // edit/save list group
    $(document).off('click', '.uniparams-save_grouplist').on('click', '.uniparams-save_grouplist', function(e) {
        e.preventDefault();

        var form_data = new FormData();
        ev_fine = true;
        $('.uniparams-oldgrouplist').find('.errormsg').html('');
        $(".uniparams-oldgrouplist :input").each(function(index) {
            that = $(this);
            if (that.attr('type') == 'file' && that.prop('files')[0]) {
                var name = that.prop('files')[0].name;
                var ext = name.split('.').pop().toLowerCase();
        ...
        if (ev_fine) {
            $.ajax({
                ...
                cache: false,
                processData: false,
                url: '?plugin=uniparams&action=savegrouplist'
            }).done(function (data) {
                if (data.status == "ok") {
                    $('.uniparams-oldgrouplist').trigger('close');
                    $('.uniparams-oldgrouplist').remove();
                    getthelist(data.data.list_id);
                } else {
                    if (data.errors.id)
                        $('.uniparams-err1').html('').html(data.errors.id);
    ...
    // delete list group
    $(document).off('click', '.uniparams-remove_grouplist').on('click', '.uniparams-remove_grouplist', function(e) {
        e.preventDefault();
        var data = $(document).find('.uniparams-oldgrouplist form').serialize();
        $.ajax({
            data: data,
            type: 'POST',
            url: '?plugin=uniparams&action=removegrouplist'
        }).done(function(){
            getthelist();
        });
        $('.uniparams-oldgrouplist').trigger('close');
    ...
    // popup edit list group
    $(document).off('click', '.uniparams-grouplist-settings').on('click', '.uniparams-grouplist-settings', function(e) {
        e.preventDefault();
        var instance = $("<div class=\"load block align-center triple-padded\"><i class=\"icon16 loading\"></i>[`Loading...`]</div>")
            .waDialog({
                onClose: function () {
        ...
        $.ajax({
            data: {'list_id': list_id},
            type: 'POST',
            url: '?plugin=uniparams&action=getgrouplistsettings'
        }).done(function(html){
            $(instance).trigger('close');
            $(html).waDialog({
                'esc': true,
    ...
    // items
    // new-item
    $(document).off('click', '.uniparams-add-item').on('click', '.uniparams-add-item', function(e) {
        e.preventDefault();
        $(".uniparams-new-item").show();
    });
    $(document).off('click', '.uniparams-cancel-new-param').on('click', '.uniparams-cancel-new-param', function(e) {
        e.preventDefault();
        $(".uniparams-new-item").hide();
    });
    $(document).off('click', '.uniparams-add-new-item').on('click', '.uniparams-add-new-item', function(e) {
        e.preventDefault();
    ...
    // preview image
    $(document).off('change', '.uniparams-itemdiv input[type="file"]').on('change', '.uniparams-itemdiv input[type="file"]', function () {
        let that = $(this);
        var oFReader = new FileReader();
        oFReader.onload = function (e) {
    ...
    // delete item
    $(document).off('click', '.uniparams-delete-item').on('click', '.uniparams-delete-item', function(e) {
        e.preventDefault();
    ...
    // edit item
    $(document).off('click', '.uniparams-edit-vals-item').on('click', '.uniparams-edit-vals-item', function(e) {
        e.preventDefault();
    ...
    // cancel item changes
    $(document).off('click', '.uniparams-cancel-edit-item').on('click', '.uniparams-cancel-edit-item', function(e) {
        e.preventDefault();
    ...
    // save item changes
    $(document).off('click', '.uniparams-save-edit-item').on('click', '.uniparams-save-edit-item', function(e) {
        e.preventDefault();
    ...
    $(document).off('change', '.uniparams-item-enabled').on('change', '.uniparams-item-enabled', function(e) {
        that = $(this);
        if (that.is(':checked')) {
            $.ajax({
    ...
    // copy to clipboard list function path
    $(document).off('click', '.uniparams-copy-to-cp').on('click', '.uniparams-copy-to-cp', function (e) {
        e.preventDefault();
        var that = $(this);
        var $temp = $("<input>");
        $("body").append($temp);
    ...
    $(document).off('click', '.uniparams-copy-to-cp-group').on('click', '.uniparams-copy-to-cp-group', function (e) {
        e.preventDefault();
        var that = $(this);
        var $temp = $("<input>");
        $("body").append($temp);
        ...
        inputv = that.parents('div').find('input[name="list_key"]').val();
        let txt = "{shopUniparamsPlugin::getGroup(\"" + inputv + "\")}";
