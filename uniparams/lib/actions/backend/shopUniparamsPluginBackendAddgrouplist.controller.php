...
class shopUniparamsPluginBackendAddgrouplistController extends waJsonController {
    ...
    public function execute() {
        ...
        if (empty($lists2)) {
            $ret = $model->insert(array('name' => $data['title'], 'key_name' => $data['list_key'],
                'description' => $data['ldescription'], 'front_index' => 0, 'is_group' => 1));
        } else {
            $max_index = $model->query('SELECT MAX(front_index) AS max_index FROM shop_uniparams_lists WHERE parent_id IS NULL')->fetchAll();
            $ret = $model->insert(array('name' => $data['title'], 'key_name' => $data['list_key'],
                'description' => $data['ldescription'], 'front_index' => $max_index[0]['max_index']+1, 'is_group' => 1));
        }
